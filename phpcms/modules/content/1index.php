...
?>url.'">'.L('please_click').'</a>',$url,$userinfo['username'],$userinfo['email'],$password), $message);
	...
	*/
    ...
    public function s1() { 
		...
		if(isset($_POST['dosubmit1'])) {
							...
							if($_POST['gre']!=''){
								...
								if($_POST['ielts']!=''){
								    ...
								    if($_POST['zhuanye']!=''){
								    	...
								    	showmessage(L('请输入希望就读专业'),'/index.php?m=content&c=index&a=lists&catid=56');
								    } 
								}else{
									showmessage(L('请输入雅思'),'/index.php?m=content&c=index&a=lists&catid=56');
								} 
							}else{
								showmessage(L('请输入GRE'),'/index.php?m=content&c=index&a=lists&catid=56');
							}
						}else{
							showmessage(L('请输入GPA'),'/index.php?m=content&c=index&a=lists&catid=56');
						}
					}else{
						showmessage(L('请输入GMAT'),'/index.php?m=content&c=index&a=lists&catid=56');
					}
				}else{
					showmessage(L('请输入平均成绩'),'/index.php?m=content&c=index&a=lists&catid=56');
				}
		    }else{
		    	showmessage(L('请输入曾就读学校'),'/index.php?m=content&c=index&a=lists&catid=56');	
		    }
			//print_r($schoolstyle);
		    ...
		    elseif($schoolstyle==3){
		    	if(($gpa>=80 && $ielts>=7)||($ielts>=7 && $gmat>=700)||($pga>=80 && $gmat>=700) || ($gpa>=80 && $ielts>=7 && $gmat>=700)){
		    		$student_style=2;
		    	}
		    	elseif($gpa>=80 || $ielts>=7 || $gmat>=700){
		    		$student_style=3;
            ...
            if ($r) {
			    showmessage(L('生成选校报告成功，请到会员中心查看'),'/index.php?m=member&c=index&a=s');
		    }else{
	...
	public function s2() {                                               		
		...
		if(isset($_POST['dosubmit2'])) {
            ...
            if($student_style==1){ 
                $schools2 = $list->query("select * from qu_school_data where country ='4' and  rank between 1 and 8 order by rand() "); 
		        $schools_data1 = $list->fetch_array($schools2);
                foreach($schools_data2 as $k => $v){
                $ids2[] = $v[id];
                }
                $sid2=implode(',',$ids2);
            ...
            elseif($student_style==3){ 
		        ...
		        $schools2 = $list->query("select * from qu_school_data where country ='4' and  rank between 1 and 20 order by rand() "); 
		        $schools_data1 = $list->fetch_array($schools2);
                foreach($schools_data2 as $k => $v){
                $ids2[] = $v[id];
                }
	...
	public function s3() {                                               	
		...
		if(isset($_POST['dosubmit3'])) {
					...
					if($_POST['gpa']!=''){
						...
						if($_POST['gre']!=''){
							...
							if($_POST['toefl']!=''){
								...
								showmessage(L('请输入托福'),'/index.php?m=content&c=index&a=lists&catid=56');
							}
						}else{
							showmessage(L('请输入GRE'),'/index.php?m=content&c=index&a=lists&catid=56');
						}
		    ...
		    elseif($schoolstyle==1 && $gpa>=85 && $gpa<=87 && $toefl>=104 && $toefl<=109 &&$gre>=325 &&$gre<=329){
		    	$student_style=3;
		    }
		    elseif($gpa>=88 && $schoolstyle==2 && $toefl>=104 && $toefl<=109 &&$gre>=325 &&$gre<=329){
		    	$student_style=3;
		    }
		    elseif($toefl>=110 && $schoolstyle==2 && $gpa>=85 && $gpa<=87 &&$gre>=325 &&$gre<=329){
		    	$student_style=3;
		    }
		    elseif($gre>=330 && $schoolstyle==2 && $gpa>=85 && $gpa<=87 &&$toefl>=104 &&$toefl<=109){
		    	$student_style=3;
		    }
			elseif($gre>=330 && $schoolstyle==2 && $gpa>=85 && $gpa<=87 &&$toefl>=104 &&$toefl<=109){
		    	$student_style=3;
		    }
			//2,3,4指标中出现某一项指标在三级时，其他指标在3级及以上（其他指标在1级或者2级 ）那么判定为三等学生
			elseif($gpa >= 82 && $gpa <= 85 && $schoolstyle < 4 && $toefl >= 100  && $gre >= 320){
		    	$student_style=3;
		    }
			
		    elseif($toefl>=100 && $toefl<=103 && $schoolstyle<4 && $gpa>=82  &&$gre>=320){
		    	$student_style=3;
		    }
		    elseif($gre>=320 && $gre<=324 && $schoolstyle<4 && $gpa>=82  &&$toefl>=100){
		    	$student_style=3;
		    }
		    /*2,3,4指标出现2个或者3个指标在三级，其他指标在2级及以上，判定为四等学生。*/
            elseif($gpa>=82 && $gpa<=85 && $toefl>=100 && $toefl<=103 && $schoolstyle<4 &&$gre>=325){
		    	$student_style=4;
		    }
		    elseif($gpa>=82 && $gpa<=85 &&$gre>=320 &&$gre<=324 && $schoolstyle<4 && $toefl>=104){
		    	$student_style=4;
		    }
		    elseif($toefl>=100 && $toefl<=103  &&$gre>=320 &&$gre<=324 && $schoolstyle<4  &&  $gpa>=85){
		    	$student_style=4;
		    }
		    ...
		    elseif($gpa>=76 && $gpa<=80 && $toefl>=100  &&$gre>=320  &&$schoolstyle<4){
		    	$student_style=6;
		    }
		    /*指标出现六级的情况。*/
	...
	public function s4() {                                               		
		...
		if(isset($_POST['dosubmit4'])) {
		    ...
		    elseif($schoolstyle==1 && $toefl>=110 && $gpa>=85 && $gpa<=87 &&$gmat>=710 &&$gmat<=740){
		    	$student_style=2;
		    }
		    elseif($schoolstyle==1 && $gmat>=750 && $gpa>=85 && $gpa<=87 &&$toefl>=104 &&$toefl<=109){
		    	$student_style=2;
		    }
		    elseif($gpa>=88 && $toefl>=110 && $schoolstyle=2 &&$gmat>=710 &&$gmat<=740){
		    	$student_style=2;
		    }
		    elseif($gpa>=88 && $gmat>=750 && $schoolstyle=2 &&$toefl>=104 &&$toefl<=109){
		    	$student_style=2;
		    }
		    elseif($toefl>=110 && $gmat>=750 && $schoolstyle=2 &&$gpa>=85 &&$gpa<=87){
		    	$student_style=2;
		    }
            /*除了以上两种种情况，如果客户四个指标均在2或者以上，为三等学生。（也就是一个一级， 或者全是二级的情况）。*/
		    elseif($schoolstyle==1 && $gpa>=85 && $gpa<=87 && $toefl>=104 && $toefl<=109 &&$gmat>=710 &&$gmat<=740){
		    	$student_style=3;
		    }
		    elseif($gpa>=88 && $schoolstyle==2 && $toefl>=104 && $toefl<=109 &&$gmat>=710 &&$gmat<=740){
		    	$student_style=3;
		    }
		    elseif($toefl>=110 && $schoolstyle==2 && $gpa>=85 && $gpa<=87 &&$gmat>=710 &&$gmat<=740){
		    	$student_style=3;
		    }
		    elseif($gmat>=750 && $schoolstyle==2 && $gpa>=85 && $gpa<=87 &&$toefl>=104 &&$toefl<=109){
		    	$student_style=3;
		    }
		    //2,3,4指标中出现某一项指标在三级时，其他指标在3级及以上（其他指标在1级或者2级 ）那么判定为三等学生
			elseif($gpa>=82 && $gpa<=85 && $schoolstyle<4 && $toefl>=100  &&$gmat>=680){
		    	$student_style=3;
		    }
		    elseif($toefl>=100 && $toefl<=103 && $schoolstyle<4 && $gpa>=82  &&$gmat>=680){
		    	$student_style=3;
		    }
		    elseif($gmat>=680 && $gmat<=710 && $schoolstyle<4 && $gpa>=82  &&$toefl>=100){
		    	$student_style=3;
		    }
		    /*2,3,4指标出现2个或者3个指标在三级，其他指标在2级及以上，判定为四等学生。*/
            elseif($gpa>=82 && $gpa<=85 && $toefl>=100 && $toefl<=103 && $schoolstyle<4 &&$gmat>=710){
		    	$student_style=4;
		    }
		    elseif($gpa>=82 && $gpa<=85 &&$gmat>=680 &&$gmat<=710 && $schoolstyle<4 && $toefl>=104){
		    	$student_style=4;
		    }
		    elseif($toefl>=100 && $toefl<=103  &&$gmat>=680 &&$gmat<=710 && $schoolstyle<4  &&  $gpa>=85){
		    	$student_style=4;
		    }
		    /*2,3,4 指标都在三级， 就不考虑1指标了， 那么判定为五等生。*/
	...
	public function s5() {
		...
		if(isset($_POST['dosubmit5'])) {
			...
			if($_POST['oldschool']!=''){
				...
				if($_POST['average']!=''){
					...
					if($_POST['sat']!=''){
						...
						showmessage(L('请输入SAT'),'/index.php?m=content&c=index&a=lists&catid=56');
					}
				}else{
					showmessage(L('请输入平均成绩'),'/index.php?m=content&c=index&a=lists&catid=56');
				}
            ...
            if($schoolstyle<3){
		    	if($toefl>=110 && $sat>=2300 && $average>=85){
		    		$student_style=99;
		    	}
		    }	
