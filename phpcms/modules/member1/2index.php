...
class index extends foreground {
	...
	public function logout() {
		...
		if($setting['snda_enable'] && param::get_cookie('_from')=='snda') {
			...
			showmessage(L('logout_success').$synlogoutstr,'/index.php');
		}
	}
	...
	*/
    ...
    public function s1() { 
		...
		if(isset($_POST['dosubmit1'])) {
							...
							if($_POST['gre']!=''){
								...
								if($_POST['ielts']!=''){
								    ...
								    if($_POST['zhuanye']!=''){
								    	...
								    	showmessage(L('请输入希望就读专业'),'/index.php?m=school&c=search');
								    } 
								}else{
									showmessage(L('请输入雅思'),'/index.php?m=school&c=search');
								} 
							}else{
								showmessage(L('请输入GRE'),'/index.php?m=school&c=search');
							}
						}else{
							showmessage(L('请输入GPA'),'/index.php?m=school&c=search');
						}
					}else{
						showmessage(L('请输入GMAT'),'/index.php?m=school&c=search');
					}
				}else{
					showmessage(L('请输入平均成绩'),'/index.php?m=school&c=search');
				}
		    }else{
		    	showmessage(L('请输入曾就读学校'),'/index.php?m=school&c=search');	
		    }
			//print_r($schoolstyle);
            ...
            if ($r) {
			    showmessage(L('生成选校报告成功，请到会员中心查看'),'/index.php?m=member');
		    }else{
					...
					if($_POST['gpa']!=''){
						...
						if($_POST['gre']!=''){
							...
							if($_POST['toefl']!=''){
								...
								showmessage(L('请输入托福'),'/index.php?m=school&c=search');
							}
						}else{
							showmessage(L('请输入GRE'),'/index.php?m=school&c=search');
						}
			...
			if($_POST['oldschool']!=''){
				...
				if($_POST['average']!=''){
					...
					if($_POST['sat']!=''){
						...
						showmessage(L('请输入SAT'),'/index.php?m=school&c=search');
					}
				}else{
					showmessage(L('请输入平均成绩'),'/index.php?m=school&c=search');
				}
