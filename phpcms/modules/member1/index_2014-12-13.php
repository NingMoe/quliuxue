...
class index extends foreground {
	...
	*/
    ...
    public function s1() { 
		...
		include template('member', 'person_report');                    //调用auto里面的index首页模版	
	}
	public function s2() {                                               //默认方法		
	    pc_base::load_sys_class('form', '', 0);
		$list=pc_base::load_model('school_data_model');
		if(isset($_POST['dosubmit2'])) {
		   ...
		   elseif($schoolstyle>2){
            ...
            if($student_style=1){ 
                $schools1 = $list->query("select * from qu_school_data where country ='4' and  rank between 1 and 8 order by rand() "); 
		        $schools_data1 = $list->fetch_array($schools1);
                
              
            }
            elseif($student_style=2){ 
                $schools1 = $list->query("select * from qu_school_data where country ='4' and  rank between 1 and 20  order by rand() limit 8 "); 
		        $schools_data1 = $list->fetch_array($schools1);
		...
		include template('content', 'person_report');                    //调用auto里面的index首页模版	
	}
	public function s3() {                                               //默认方法		
	    pc_base::load_sys_class('form', '', 0);
		$list=pc_base::load_model('school_data_model');
