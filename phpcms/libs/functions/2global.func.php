 ...
 */
function front_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
...
function pageurl($urlrule, $page, $array = array()) {
	...
	$urlrule.='&diqu='.$_GET[diqu].'&guojia='.$_GET[guojia].'';
...
function get_email($userid='', $field='') {
	...
	if(is_numeric($userid)) {
		...
		if(!empty($field) && $field != 'email' && isset($memberinfo[$field]) &&!empty($memberinfo[$field])) {

			$return = $memberinfo[$field];

		} else {

			$return = isset($memberinfo['email']) && !empty($memberinfo['email']) ? $memberinfo['email'].'('.$memberinfo['email'].')' : $memberinfo['email'];

		}

	} else {

		if (param::get_cookie('_email')) {

			$return .= '('.param::get_cookie('_email').')';
...
function get_mobile($userid='', $field='') {
	...
	if(is_numeric($userid)) {
		...
		if(!empty($field) && $field != 'mobile' && isset($memberinfo[$field]) &&!empty($memberinfo[$field])) {

			$return = $memberinfo[$field];

		} else {

			$return = isset($memberinfo['mobile']) && !empty($memberinfo['mobile']) ? $memberinfo['mobile'].'('.$memberinfo['mobile'].')' : $memberinfo['mobile'];

		}

	} else {

		if (param::get_cookie('_mobile')) {

			$return .= '('.param::get_cookie('_mobile').')';
...
function jiamiannum($pid) {
		...
		$rs = $list2->query("SELECT b.id FROM qu_professional_data a,qu_school_professional_data b where a.id=b.direction_pid and a.professional_pid=$pid ORDER BY b.id DESC");
...
function jiamiannum2($cid) {

        pc_base::load_sys_class('form','',0);

        $list=pc_base::load_model('school_professional_data_model');

		$rs = $list->query("SELECT id FROM qu_school_professional_data  where direction_pid=$cid ORDER BY id DESC");
...
function jiamianall($pid) {
		...
		$rs = $list->query("SELECT id FROM qu_professional_data  where professional_pid=$pid ORDER BY id DESC");
...
function jiamian_school($id,$field) {
		...
		$rs = $list->query("SELECT * FROM qu_school_data  where id=$id ORDER BY id DESC");
...
function jiamian_name($id) {
		...
		$rs = $list->query("SELECT content FROM qu_professional_data where id=$id ORDER BY id DESC");
...
function anli_thumb($id) {

        pc_base::load_sys_class('form','',0);

        $list=pc_base::load_model('successful_case_model');

		

		$rs = $list->query("SELECT thumb FROM qu_successful_case where id=$id ORDER BY id DESC");

        $data = $list->fetch_array($rs);

        

		$content=$data[0][thumb];
...
function anli_title($id) {
		...
		$rs = $list->query("SELECT title FROM qu_successful_case where id=$id ORDER BY id DESC");
...
function anli_announce($id) {

        pc_base::load_sys_class('form','',0);

        $list=pc_base::load_model('announce_model');

		

		$rs = $list->query("SELECT content FROM qu_announce where aid=$id ORDER BY aid DESC");
...
function anli_agency($id) {
		...
		if($id!=''){
			...
			$rs = $list->query("SELECT title FROM qu_intermediary where id=$id ORDER BY id DESC");
...
function bjts($id,$field) {
		...
		if($id!=''){
			...
			$rs = $list->query("SELECT $field FROM qu_xiangmu where id=$id ORDER BY id DESC");
...
function cpdz($id,$field) {

        pc_base::load_sys_class('form','',0);

        $list=pc_base::load_model('chanpin_model');

		if($id!=''){

			$rs = $list->query("SELECT $field FROM qu_chanpin where id=$id ORDER BY id DESC");
...
function dianji11($id) {

                   pc_base::load_sys_class('form', '', 0);

		           $db = pc_base::load_model('hits_model'); 

                   $_r = $db->get_one(array('hitsid'=>'c-22-'.$id)); 
...
function dianji12($id) {

                   pc_base::load_sys_class('form', '', 0);

		           $db = pc_base::load_model('hits_model'); 

                   $_r = $db->get_one(array('hitsid'=>'c-23-'.$id)); 
